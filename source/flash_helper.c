...
int flash_program(uint32_t offset, void *data, size_t size)
{
    ...
    PRINTF("\nFLASH HELPER::Programming %u bytes of flash from offset 0x%x", size, offset);
...
{
    PRINTF("\nFLASH HELPER::flash_sha256()::Calculating SHA-256 of %u bytes of flash from offset 0x%x:	", size, offset);
    uint32_t buf[128 / sizeof(uint32_t)];
    status_t status;
    mbedtls_sha256_context sha256_ctx;

    mbedtls_sha256_init(&sha256_ctx);
    mbedtls_sha256_starts_ret(&sha256_ctx, 0); // The '0' parameter means use SHA-256, not SHA-224
    ...
    while (size > 0)
    {
        ...
        mbedtls_sha256_update_ret(&sha256_ctx, (unsigned char *)buf, chunkAlign4);
