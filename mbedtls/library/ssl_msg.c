...
int mbedtls_ssl_write_handshake_msg(mbedtls_ssl_context *ssl)
{
	...
	mbedtls_printf("=> write handshake message ssl->out_msglen: %d\n", ssl->out_msglen);
    /*
     * Sanity checks
     */
    if (ssl->out_msgtype != MBEDTLS_SSL_MSG_HANDSHAKE          &&
        ...
        {
            MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
			mbedtls_printf("should never happen ssl->out_msgtype: %d\n", ssl->out_msgtype);
            return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
        }
    }
    ...
    if (!(ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE &&
        ...
        ssl->handshake == NULL) {
		mbedtls_printf("Should never happen: ssl->out_msgtype: %d, hs_type: %d\n", ssl->out_msgtype, hs_type);
        MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        ssl->handshake->retransmit_state == MBEDTLS_SSL_RETRANS_SENDING) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
		mbedtls_printf("Should never happen: ssl->handshake :%d ssl->conf->transport :%d ssl->handshake->retransmit_state: %d\n", ssl->handshake, ssl->conf->transport, ssl->handshake->retransmit_state);
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }
#endif
    ...
    if (ssl->out_msglen > MBEDTLS_SSL_OUT_CONTENT_LEN) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("Record too large: "
                                  ...
                                  ssl->out_msglen,
                                  (size_t) MBEDTLS_SSL_OUT_CONTENT_LEN));
        mbedtls_printf("Record too large: size %" MBEDTLS_PRINTF_SIZET ", maximum %" MBEDTLS_PRINTF_SIZET "\n", ssl->out_msglen, (size_t) MBEDTLS_SSL_OUT_CONTENT_LEN);	
        return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
    }
    ...
    if (ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) {
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
            ...
            if (MBEDTLS_SSL_OUT_CONTENT_LEN - ssl->out_msglen < 8) {
                ...
                mbedtls_printf("DTLS handshake message too large: size %" MBEDTLS_PRINTF_SIZET ", maximum %" MBEDTLS_PRINTF_SIZET "\n", hs_len, (size_t) (MBEDTLS_SSL_OUT_CONTENT_LEN - 12));
                return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
            }
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        if ((ret = ssl_flight_append(ssl)) != 0) {
            ...
            mbedtls_printf("ssl_flight_append failed ret = %d\n", ret);
            return ret;
        }
    } else
#endif
    {
        if ((ret = mbedtls_ssl_write_record(ssl, SSL_FORCE_FLUSH)) != 0) {
            ...
            mbedtls_printf("ssl_write_record failed ret = %d\n", ret);
            return ret;
        }
    }

    MBEDTLS_SSL_DEBUG_MSG(2, ("<= write handshake message"));
    mbedtls_printf("write handshake msg");
    return 0;
}
...
int mbedtls_ssl_write_record(mbedtls_ssl_context *ssl, uint8_t force_flush)
{
    ...
    if (!done) {
        ...
        if (ssl->transform_out != NULL) {
            ...
            if ((ret = mbedtls_ssl_encrypt_buf(ssl, ssl->transform_out, &rec,
                ...
                mbedtls_printf("ssl_encrypt_buf failed ret = %d\n", ret);
                return ret;
            }

            if (rec.data_offset != 0) {
                MBEDTLS_SSL_DEBUG_MSG(1, ("should never happen"));
                mbedtls_printf("rec.data_offset != 0 should never happen\n");
                return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
            }
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
            ret = ssl_get_remaining_space_in_datagram(ssl);
            mbedtls_printf("remaining space in datagram = %d\n", ret);
            if (ret < 0) {
                return ret;
            }
        ...
        if (i == mbedtls_ssl_ep_len(ssl)) {
            ...
            mbedtls_printf("outgoing message counter would wrap\n");
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ...
        size_t remaining;
        ret = ssl_get_remaining_payload_in_datagram(ssl);
        mbedtls_printf("remaining payload in datagram = %d\n", ret);
        if (ret < 0) {
    ...
    if ((flush == SSL_FORCE_FLUSH) &&
        ...
        mbedtls_printf("mbedtls_ssl_flush_output failed ret = %d\n", ret);
        return ret;
    }
...
#endif
mbedtls_printf("\n mbedtls_ssl_update_handshake_status executed");
}
                            ...
                            unsigned update_hs_digest)
{
    ...
    if (ssl->keep_current_message == 0) {
        do {
            ...
            if (ret != 0) {
                mbedtls_printf("\n ssl_consume_current_message : ret = %d", ret);
                return ret;
            ...
            mbedtls_printf("\n mbedtls_ssl_handle_message_type : ret = %d", ret);
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("<= read record"));
    mbedtls_printf("\n mbedtls_ssl_read_record : Success");
    return 0;
}
    ...
    if (ssl->in_hslen != 0) {
        ...
        if (ssl->in_hslen < ssl->in_msglen) {
            ...
            MBEDTLS_SSL_DEBUG_BUF(4, "remaining content in record",
                                  ssl->in_msg, ssl->in_msglen);
            mbedtls_printf("\n remaining content in record : %s", ssl->in_msg);
        } else {
            ssl->in_msglen = 0;
        }
...
int mbedtls_ssl_write_change_cipher_spec(mbedtls_ssl_context *ssl)
{
    ...
    if ((ret = mbedtls_ssl_write_handshake_msg(ssl)) != 0) {
        ...
        mbedtls_printf("mbedtls_ssl_write_change_cipher_spec()::mbedtls_ssl_write_handshake_msg returned %d\n", ret);
        return ret;
    }
