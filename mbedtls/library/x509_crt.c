                           ...
                           size_t buflen)
{
    ...
    if (buflen != 0 && buf[buflen - 1] == '\0' &&
        ...
        mbedtls_printf("\nmbedtls_x509_crt_parse  Cert format PEM");
        buf_format = MBEDTLS_X509_FORMAT_PEM;
    }

    if (buf_format == MBEDTLS_X509_FORMAT_DER) {
        mbedtls_printf("\nmbedtls_x509_crt_parse  Cert format DER");
    ...
    if (buf_format == MBEDTLS_X509_FORMAT_PEM) {
        ...
        mbedtls_printf("\nmbedtls_x509_crt_parse  Cert format PEM : Buffer size %d", buflen);
        ...
        while (buflen > 1) {
            ...
            mbedtls_printf("\nmbedtls_x509_crt_parse  Cert Parse der : ret %d", ret);
            mbedtls_pem_free(&pem);
 ...
 * of CRLs.
 */
void ip_string_to_binary(const char *ip_string, unsigned char *ip_binary) {
    int a, b, c, d;
                                             ...
                                             mbedtls_x509_crt_restart_ctx *rs_ctx)
{
    ...
    *flags = 0;
    ...
    /* Check the SAN field if the CN check failed */
    if (ee_flags & MBEDTLS_X509_BADCERT_CN_MISMATCH) {
        mbedtls_x509_sequence *san = &crt->subject_alt_names;

        while (san != NULL) {
            /* Convert the IP address from string to binary */
            unsigned char ip_binary[16];
            size_t ip_binary_len = sizeof(ip_binary);
            //mbedtls_net_pton(cn, ip_binary);
            ip_string_to_binary(cn, ip_binary);

            /* Check if the SAN field contains the IP address */
            if (san->buf.len == ip_binary_len && memcmp(san->buf.p, ip_binary, san->buf.len) == 0) {
                /* Clear the CN mismatch flag if the IP address is found */
                ee_flags &= ~MBEDTLS_X509_BADCERT_CN_MISMATCH;
                break;
            }
