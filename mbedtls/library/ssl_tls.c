                                  ...
                                  mbedtls_ssl_context *ssl)
{
    ...
    mbedtls_printf("\n Ciphersuite Info Cipher:%d, Mac: %d, Key Exchange:%d", ciphersuite_info->cipher, ciphersuite_info->mac, ciphersuite_info->key_exchange);
    if (ciphersuite_info == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("ciphersuite info for %d not found",
                                  ciphersuite));
        mbedtls_printf("\n ciphersuite info for %d not found", ciphersuite);
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }

    cipher_info = mbedtls_cipher_info_from_type(ciphersuite_info->cipher);
    mbedtls_printf("\n Cipher Info Block Size:%d, Key Size:%d, IV Size:%d", cipher_info->block_size, cipher_info->key_bitlen, cipher_info->iv_size);
    if (cipher_info == NULL) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("cipher info for %u not found",
                                  ciphersuite_info->cipher));
        mbedtls_printf("\n cipher info for %u not found", ciphersuite_info->cipher);
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }

    md_info = mbedtls_md_info_from_type(ciphersuite_info->mac);

    mbedtls_printf("\n MD Info Name:%s, Type:%d, Size:%d", 
    ...
    if (ssl->handshake->cid_in_use == MBEDTLS_SSL_CID_ENABLED) {
        ...
        mbedtls_printf("\n Copying own and peer's CID if the use of the CID extension has been negotiated");
    }
#endif /* MBEDTLS_SSL_DTLS_CONNECTION_ID */
    ...
    if (cipher_info->mode == MBEDTLS_MODE_STREAM ||
        ...
        if ((ret = mbedtls_md_setup(&transform->md_ctx_enc, md_info, 1)) != 0 ||
            ...
            mbedtls_printf("\n Init HMAC contexts for CBC/Stream mbedtls_md_setup failed");
            goto end;
        }
    ...
    if (minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1) {
        ...
        if (mac_key_len != 0) {
            ret = mbedtls_md_hmac_starts(&transform->md_ctx_enc,
                                         mac_enc, mac_key_len);
            if (ret != 0) {
                mbedtls_printf("\n mbedtls_md_hmac_starts() ENC : failed with error : %d", ret);
                goto end;
            }
            ret = mbedtls_md_hmac_starts(&transform->md_ctx_dec,
                                         mac_dec, mac_key_len);
            if (ret != 0) {
                mbedtls_printf("\n mbedtls_md_hmac_starts() DEC : failed with error : %d", ret);
                goto end;
            }
        }
    } else
    ...
    if (mbedtls_ssl_hw_record_init != NULL) {
        ...
        if ((ret = mbedtls_ssl_hw_record_init(ssl, key1, key2, keylen,
            ...
            mbedtls_printf("\n mbedtls_ssl_hw_record_init() failed with error : %d", ret);
    ...
    if (minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
        ...
        if (ret == 0) {
            ...
            mbedtls_printf("\n Successfully setup PSA-based encryption cipher context");
            psa_fallthrough = 0;
        } else {
    ...
    if (do_mbedtls_cipher_setup &&
        ...
        mbedtls_printf("\n mbedtls_cipher_setup() failed with error : %d", ret);
        goto end;
    }
    ...
    if (minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
        ...
        if (ret != 0 && ret != MBEDTLS_ERR_CIPHER_FEATURE_UNAVAILABLE) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_setup_psa", ret);
            mbedtls_printf("\n TLS v1.2 mbedtls_cipher_setup_psa() failed with error : %d", ret);
            goto end;
        }

        if (ret == 0) {
            MBEDTLS_SSL_DEBUG_MSG(3, ("Successfully setup PSA-based decryption cipher context"));
            mbedtls_printf("\n TLS v1.2 Successfully setup PSA-based decryption cipher context");
            psa_fallthrough = 0;
        } else {
            MBEDTLS_SSL_DEBUG_MSG(1,
                                  (
                                      "Failed to setup PSA-based cipher context for record decryption - fall through to default setup."));
            mbedtls_printf("\n TLS v1.2 Failed to setup PSA-based cipher context for record decryption - fall through to default setup.");
            psa_fallthrough = 1;
        }
    } else {
        psa_fallthrough = 1;
    ...
    if ((ret = mbedtls_cipher_setkey(&transform->cipher_ctx_enc, key1,
        ...
        mbedtls_printf("\n mbedtls_cipher_setkey() Enc : failed with error : %d", ret);
        goto end;
    }
    ...
    if ((ret = mbedtls_cipher_setkey(&transform->cipher_ctx_dec, key2,
                                     ...
                                     MBEDTLS_DECRYPT)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_setkey", ret);
        mbedtls_printf("\n mbedtls_cipher_setkey() Dec: failed with error : %d", ret);
        goto end;
    }
    ...
    if (cipher_info->mode == MBEDTLS_MODE_CBC) {
        if ((ret = mbedtls_cipher_set_padding_mode(&transform->cipher_ctx_enc,
            ...
            mbedtls_printf("\n mbedtls_cipher_set_padding_mode() Enc failed with error : %d", ret);
            goto end;
        }
        ...
        if ((ret = mbedtls_cipher_set_padding_mode(&transform->cipher_ctx_dec,
                                                   MBEDTLS_PADDING_NONE)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_cipher_set_padding_mode", ret);
            mbedtls_printf("\n mbedtls_cipher_set_padding_mode() Dec failed with error : %d", ret);
            goto end;
        }
    }
                                  ...
                                  mbedtls_md_type_t hash)
{
    ...
    mbedtls_printf("ssl_set_handshake_prfs()::tls_prf : %s, calc_verify : %s, calc_finished : %s\n", handshake->tls_prf, handshake->calc_verify, handshake->calc_finished);
    return 0;
}
                              ...
                              const mbedtls_ssl_context *ssl)
{
    ...
    if (handshake->resume != 0) {
        ...
        mbedtls_printf("no premaster (session resumed)\n");
        return 0;
    }
    ...
    if (handshake->extended_ms == MBEDTLS_SSL_EXTENDED_MS_ENABLED) {
        ...
        mbedtls_printf("session hash for extended master secret : %s, length : %d \n", session_hash, salt_len);
    }
    ...
    if (handshake->ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_PSK &&
        ...
        mbedtls_printf("perform PSA-based PSK-to-MS expansion\n");
        ...
        if (status != PSA_SUCCESS) {
            psa_key_derivation_abort(&derivation);
            mbedtls_printf("setup_psa_key_derivation failed with %d\n", status);
            return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
        }
        ...
        if (status != PSA_SUCCESS) {
            psa_key_derivation_abort(&derivation);
            mbedtls_printf("psa_key_derivation_output_bytes failed with %d\n", status);
            return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
        }

        status = psa_key_derivation_abort(&derivation);
        if (status != PSA_SUCCESS) {
            mbedtls_printf("psa_key_derivation_abort failed with %d\n", status);
            return MBEDTLS_ERR_SSL_HW_ACCEL_FAILED;
        }
    } else
#endif
    {
        ret = handshake->tls_prf(handshake->premaster, handshake->pmslen,
                                 ...
                                 master_secret_len);
        if (ret != 0) {
            mbedtls_printf("tls_prf failed with %d\n", ret);
            MBEDTLS_SSL_DEBUG_RET(1, "prf", ret);
            return ret;
        }
        ...
        MBEDTLS_SSL_DEBUG_BUF(3, "premaster secret",
                              handshake->premaster,
                              handshake->pmslen);
        mbedtls_printf("premaster secret : %s, length : %d \n", handshake->premaster, handshake->pmslen);
...
int mbedtls_ssl_derive_keys(mbedtls_ssl_context *ssl)
{
    ...
    if (ret != 0) {
        ...
        mbedtls_printf("ssl_set_handshake_prfs failed with %d\n", ret);	
        return ret;
    }
    ...
    if (ret != 0) {
        ...
        mbedtls_printf("ssl_compute_master failed with %d\n", ret);
        return ret;
    }
    ...
    if (ret != 0) {
        ...
        mbedtls_printf("ssl_populate_transform failed with %d\n", ret);
        return ret;
    }
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("<= derive keys"));
    mbedtls_printf("derive keys success\n");
    return 0;
}
...
{
    int ret = 0;
    mbedtls_printf("\n ssl_parse_certificate_verify : SSL Context: %p Authmode : %d Chain : %p RS Context : %p", ssl, authmode, chain, rs_ctx); 
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->handshake->ciphersuite_info;
    int have_ca_chain = 0;
...
#endif /* MBEDTLS_X509_TRUSTED_CERTIFICATE_CALLBACK */
    {
        ...
        if (ssl->handshake->sni_ca_chain != NULL) {
            ...
            mbedtls_printf("\n Using SNI CA Chain CAChain : %p CRL : %p", ca_chain, ca_crl);
        } else
#endif
        {
            ca_chain = ssl->conf->ca_chain;
            ca_crl   = ssl->conf->ca_crl;
            mbedtls_printf("\n Using Default CA Chain CAChain : %p CRL : %p", ca_chain, ca_crl);
        }
        ...
        mbedtls_printf("\n mbedtls_x509_crt_verify_restartable ret = %d", ret);
    }
    ...
    if (authmode == MBEDTLS_SSL_VERIFY_OPTIONAL &&
        (ret == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED ||
         ret == MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE)) {
        ret = 0;
        mbedtls_printf("\n ret = 0 in ssl_parse_certificate_verify");
    }

    if (have_ca_chain == 0 && authmode == MBEDTLS_SSL_VERIFY_REQUIRED) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("got no CA chain"));
        ret = MBEDTLS_ERR_SSL_CA_CHAIN_REQUIRED;
        mbedtls_printf("\n got no CA chain ret = %d", ret);
    }
    ...
    if (ret != 0) {
        ...
        mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                       alert);
        mbedtls_printf("\n mbedtls_ssl_send_alert_message ALERT_LEVEL_FATAL: Alert %d", alert);
    }
    ...
    if (ssl->session_negotiate->verify_result != 0) {
        ...
        mbedtls_printf("\n Certificate verification flags %08x",
                       (unsigned int) ssl->session_negotiate->verify_result);
    } else {
...
int mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    ...
    mbedtls_printf("\n ssl_parse_certificate_chain: ret = %d", ret);
    if (ret != 0) {
        goto exit;
    }
...
crt_verify:
    ...
    ret = ssl_parse_certificate_verify(ssl, authmode,
                                       chain, rs_ctx);
    mbedtls_printf("\n ssl_parse_certificate_verify: ret = %d", ret);
    if (ret != 0) {
        goto exit;
    }
...
exit:
    ...
    mbedtls_printf("\n mbedtls_ssl_parse_certificate ret = %d", ret);
    return ret;
}
...
int mbedtls_ssl_set_hostname(mbedtls_ssl_context *ssl, const char *hostname)
{
    ...
    if (hostname == NULL) {
        ...
        mbedtls_printf("\nhostname: %s Length: %d", ssl->hostname, hostname_len);
...
#endif
    mbedtls_printf("\nmbedtls_ssl_handshake_step() ret = %d", ret);
    return ret;
}
...
int mbedtls_ssl_handshake(mbedtls_ssl_context *ssl)
{
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("<= handshake"));
    mbedtls_printf("\n<= handshake");
    mbedtls_printf("\nmbedtls_ssl_handshake() ret = %d", ret);
    return ret;
}
