                         ...
                         const mbedtls_cipher_info_t *cipher_info)
{
    ...
    if (cipher_info == NULL) {
    	mbedtls_printf("cipher_info is NULL\n");
    ...
    if (NULL == (ctx->cipher_ctx = cipher_info->base->ctx_alloc_func())) {
        mbedtls_printf("ctx->cipher_ctx is NULL\n");
                          ...
                          const mbedtls_operation_t operation)
{
    ...
    if (ctx->cipher_info == NULL) {
    	 mbedtls_printf("mbedtls_cipher_setkey:ctx->cipher_info is NULL\n");
        return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
    }
    ...
    if (ctx->psa_enabled == 1) {
        ...
        if (key_bitlen % 8 != 0) {
			 mbedtls_printf("mbedtls_cipher_setkey:key_bitlen is not byte-aligned\n");
            return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
        }

        /* Don't allow keys to be set multiple times. */
        if (cipher_psa->slot_state != MBEDTLS_CIPHER_PSA_KEY_UNSET) {
			mbedtls_printf("mbedtls_cipher_setkey:cipher_psa->slot_state is not MBEDTLS_CIPHER_PSA_KEY_UNSET\n");
            return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
        }

        key_type = mbedtls_psa_translate_cipher_type(
            ctx->cipher_info->type);
		mbedtls_printf("mbedtls_cipher_setkey: key_type:%d\n", key_type);
        if (key_type == 0) {
			mbedtls_printf("mbedtls_cipher_setkey:key_type is 0\n");
            return MBEDTLS_ERR_CIPHER_FEATURE_UNAVAILABLE;
        }
        ...
        status = psa_import_key(&attributes, key, key_bytelen,
                                &cipher_psa->slot);
		mbedtls_printf("mbedtls_cipher_setkey:psa_import_key status:%d\n", status);
        switch (status) {
            case PSA_SUCCESS:
                break;
    ...
    if ((ctx->cipher_info->flags & MBEDTLS_CIPHER_VARIABLE_KEY_LEN) == 0 &&
		...
		mbedtls_printf("mbedtls_cipher_setkey:ctx->cipher_info->key_bitlen is not equal to key_bitlen\n");
        return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
    }
	...
	mbedtls_printf("mbedtls_cipher_setkey:ctx->cipher_info->mode is not MBEDTLS_MODE_CFB\n");
    return MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA;
}
...
#endif /* MBEDTLS_USE_PSA_CRYPTO */
	mbedtls_printf("mbedtls_cipher_set_padding_mode:mode:%d\n", mode);
    switch (mode) {
#if defined(MBEDTLS_CIPHER_PADDING_PKCS7)
